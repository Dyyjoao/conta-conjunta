...
function rows(type){
  ...
  return list.map(c=>{const used=usage(c.id),keys=Array.isArray(c.ofxKeywords)?c.ofxKeywords:[];return `<div class="cc-category-row ${c.active===false?'is-inactive':''}"><div><strong>${esc(c.name)}</strong><div class="muted">${c.active===false?'Inativa':'Ativa'}${used?' · em uso':''}${keys.length?` · OFX: ${esc(keys.join(', '))}`:''}</div></div><div class="cc-category-actions"><button class="btn ghost" data-edit="${c.id}">Editar</button><button class="btn ghost" data-toggle="${c.id}">${c.active===false?'Ativar':'Desativar'}</button><button class="btn ghost danger" data-delete="${c.id}" ${used?'disabled':''}>Excluir</button></div></div>`}).join('');
}
function render(){
...
function open(id=''){const c=categories.find(x=>x.id===id);document.getElementById('ccCategoryForm').reset();document.getElementById('ccCategoryId').value=c?.id||'';document.getElementById('ccCategoryName').value=c?.name||'';document.getElementById('ccCategoryType').value=c?.type||'expense';document.getElementById('ccCategoryKeywords').value=(c?.ofxKeywords||[]).join(', ');document.getElementById('ccCategoryActive').checked=c?.active!==false;document.getElementById('ccCategoryTitle').textContent=c?'Editar categoria':'Nova categoria';document.getElementById('ccCategoryError').textContent='';document.getElementById('ccCategoryDialog').showModal()}
async function save(e){
  e.preventDefault();const id=document.getElementById('ccCategoryId').value,name=document.getElementById('ccCategoryName').value.trim(),type=document.getElementById('ccCategoryType').value,active=document.getElementById('ccCategoryActive').checked,error=document.getElementById('ccCategoryError');
  const ofxKeywords=[...new Set(document.getElementById('ccCategoryKeywords').value.split(',').map(x=>norm(x)).filter(x=>x.length>=2))];error.textContent='';
  if(!name){error.textContent='Informe o nome.';return}if(categories.some(c=>c.id!==id&&c.type===type&&norm(c.name)===norm(name))){error.textContent='Já existe uma categoria com esse nome e tipo.';return}
  try{if(id)await f.updateDoc(f.doc(db,'households',hid,'categories',id),{name,type,active,ofxKeywords,updatedAt:f.serverTimestamp()});else await f.addDoc(f.collection(db,'households',hid,'categories'),{name,type,active,ofxKeywords,systemDefault:false,createdBy:user.uid,createdAt:f.serverTimestamp(),updatedAt:f.serverTimestamp()});document.getElementById('ccCategoryDialog').close()}catch(err){console.error(err);error.textContent='Não foi possível salvar a categoria.'}
}
