...
let f,db,auth,hid,accounts=[],transactions=[],unsubs=[];
...
function render(){
  ...
  const page=document.getElementById('pageContent'),base=page?.querySelector(':scope > .kpi-grid');if(!base||page.querySelector('#ccProjection'))return;
  ...
  const active=accounts.filter(a=>a.active!==false),total=active.reduce((s,a)=>s+balance(a),0),invested=active.filter(a=>a.type==='investment').reduce((s,a)=>s+balance(a),0),available=total-invested;
  ...
  ${kpi('Saldo projetado',money(total+pi-pe),'Saldo atual + previsões')}</div></section>
  <section class="cc-enh-block"><div class="cc-section-title"><div><div class="eyebrow">POSIÇÃO FINANCEIRA</div><h2>Saldos e investimentos</h2></div></div><div class="kpi-grid">
  ${kpi('Patrimônio financeiro',money(total),'Contas + investimentos')}
  ${kpi('Saldo disponível',money(available),'Líquido de investimentos')}
  ${kpi('Investimentos',money(invested),'Contas tipo investimento')}
